...
const mode = ref<'single' | 'dual' | 'free'>('single')
...
function generateObstacles(pl: Player) {
  ...
  if (!hellMode && mode.value !== 'free') return
  ...
  const count = mode.value === 'free' ? getFreeObstacleCount(pl.score) : 3 + Math.floor(Math.random() * 3)
...
function reset() {
  ...
  started.value = false
  obstacles.value = []
  if (mode.value === 'single' || mode.value === 'free') {
...
function tick() {
  ...
  players.value.forEach(pl => {
    ...
    if (obstacles.value.some(o => o.x === next.x && o.y === next.y)) { pl.gameOver = true; return }
  ...
  players.value.forEach(pl => {
    ...
    if ((mode.value === 'single' && DIFFICULTIES[difficulty.value]!.hasObstacles) || mode.value === 'free') generateObstacles(pl)
    spawnFoods(pl)
  })
  lastTick = performance.now()
  curInt = mode.value === 'free' ? getFreeSpeed(players.value[0]!.score) : 95
  timer = setTimeout(tick, curInt)
}

function switchMode(m: 'single' | 'dual' | 'free') {
  if (timer) clearTimeout(timer)
  mode.value = m
  if (m === 'single' || m === 'free') {
    players.value = [makePlayer(10, 10, DIR.RIGHT)]
  ...
  <div class="game-container">
    ...
    <div class="sidebar">
      ...
      <div class="controls">
        ...
        <p class="arrow-keys" v-if="mode === 'single' || mode === 'free'"><kbd>&uarr;&darr;&larr;&rarr;</kbd></p>
